...
HRESULT OLEWrap2(WORD autoType, VARIANT *pvResult, IDispatch *pDispatch, LPOLESTR ptName, DWORD cArgs...)
{
	if ( !pDispatch )
	{
		assert(!"NULL IDispatch passed to OLEWrap2");
		return E_FAIL;
	}
...
COLEAutoExcelWrapper2::COLEAutoExcelWrapper2()
{
	m_hRes = S_OK;
	m_pExcelApp = NULL;
	m_pExcelBooks = NULL;
...
HRESULT COLEAutoExcelWrapper2::Quit()
{
	if ( !m_pExcelApp )
	{
		return E_FAIL;
...
HRESULT COLEAutoExcelWrapper2::Initialize()
{
	CoInitialize(NULL);
	CLSID clsid;
...
HRESULT COLEAutoExcelWrapper2::SetVisible( bool bVisible )
{
	if ( !m_pExcelApp )
	{
		return E_FAIL;
	...
	m_hRes = OLEWrap2( DISPATCH_PROPERTYPUT, NULL, m_pExcelApp, L"Visible", 1, param1 );

	return m_hRes;
}

HRESULT COLEAutoExcelWrapper2::OpenExcelBook( bool bVisible )
{
	if ( !m_pExcelApp ) 
	{
...

	{
		VARIANT result;
		VariantInit(&result);
		m_hRes = OLEWrap2(DISPATCH_PROPERTYGET, &result, m_pExcelApp, L"Workbooks", 0);
		if ( FAILED(m_hRes) )
		{
			return m_hRes;
		}
...

	{
		...
		m_hRes = OLEWrap2(DISPATCH_PROPERTYGET, &result, m_pExcelBooks, L"Open", 1, param1);
...

	{
		...
		m_hRes = OLEWrap2(DISPATCH_PROPERTYGET, &result, m_pActiveBook, L"Sheets", 0);
		if ( FAILED(m_hRes) )
		{
			return m_hRes;
		}
...
HRESULT COLEAutoExcelWrapper2::Save()
{
	if ( !m_pActiveBook )
	{
		return E_FAIL;
	...
	m_hRes = OLEWrap2( DISPATCH_METHOD, NULL, m_pActiveBook, L"Save", 0 );

	return m_hRes;
}

void COLEAutoExcelWrapper2::SetFilePath(const wxString& sFilePath )
{
	m_sFilePath = sFilePath;
}

HRESULT COLEAutoExcelWrapper2::GetExcelSheets( Atlas::Vector<wxString>& vSheets )
{
	if ( !m_pExcelSheets )
	{
		return E_FAIL;
	...
	DWORD iSheetCount;
	{
		VARIANT result;
		VariantInit(&result);
		m_hRes = OLEWrap2(DISPATCH_PROPERTYGET, &result, m_pExcelSheets, L"Count", 0);
		if ( FAILED(m_hRes) )
		{
			return m_hRes;
		}
	...
	for ( DWORD ii = 0; ii < iSheetCount; ++ii )
	{
		IDispatch *pSheet;
		{
			...
			param1.vt = VT_I4;
			param1.lVal = ii + 1;
			m_hRes = OLEWrap2(DISPATCH_PROPERTYGET, &result, m_pExcelSheets, L"Item", 1, param1);
			if ( FAILED(m_hRes) )
			{
				return m_hRes;
			}
...

		{
			VARIANT result;
			VariantInit(&result);
			m_hRes = OLEWrap2(DISPATCH_PROPERTYGET, &result, pSheet, L"Name", 0);
			if ( FAILED(m_hRes) )
			{
				return m_hRes;
			}
...
HRESULT COLEAutoExcelWrapper2::SetActiveSheet( const wxString& sSheetName )
{
	if ( !m_pExcelSheets )
	{
		return E_FAIL;
...
HRESULT COLEAutoExcelWrapper2::SetCellValue( const wxString& sRange, const wxString& sValue )
{
	if ( !m_pActiveSheet )
	{
		return E_FAIL;
	...
	IDispatch* pRange;
	{
		...
		VARIANT result;
		VariantInit(&result);
		m_hRes = OLEWrap2(DISPATCH_PROPERTYGET, &result, m_pActiveSheet, L"Range", 1, param1);
		SysFreeString( param1.bstrVal );
		if ( FAILED(m_hRes) )
		{
			return m_hRes;
...

	{
		...
		m_hRes = OLEWrap2(DISPATCH_PROPERTYPUT, NULL, pRange, L"Value", 1, param1);
		SysFreeString( param1.bstrVal );
	}
...
HRESULT COLEAutoExcelWrapper2::GetCellValue( const wxString& sRange, wxString& sValue )
{
	if ( !m_pActiveSheet )
	{
		return E_FAIL;
...

	{
		VARIANT result;
		VariantInit(&result);
		m_hRes = OLEWrap2(DISPATCH_PROPERTYGET, &result, pRange, L"Value", 0);
		if ( FAILED(m_hRes) )
		{
			return m_hRes;
		}
