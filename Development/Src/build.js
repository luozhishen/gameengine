...
AppBuilder.prototype.setPlatform = function (platform) {
	if(platform=='win32') {
		...
		this.dll_ext = '.so';
		this.lib_ext = '.a';
		this.cd_exe = 'g++ -M {1} {0} -c -o {2}';
		this.cc_exe = 'g++ -D_LINUX {1} {0} -c -o {2}';
		this.ld_exe = 'g++ -static {0} {2} -o {1}';
...
AppBuilder.prototype.setConfiguration = function (config) {
	if(config=='debug') {
		...
		this.cc_flag = '-g -D__x86_64__ -fpermissive';
		this.ln_flag = '';
		this.sl_flag = '';
		this.dl_flag = '';
		this.config = config;
	...
	if(config=='release') {
		...
		this.cc_flag = '-O2 -D__x86_64__ -fpermissive';
		this.ln_flag = '';
		this.sl_flag = '';
		this.dl_flag = '';
		this.config = config;
...
*/
	...
	for(var i=0; i<proj.src_files.length; i++) {
		...
		objs.push(dst_path+this.obj_ext);
