...
namespace Zion
{
	...
	public:
		...
		virtual void QueryGeneralsDone(const Zion::Array<SG_GENERAL>& generals) = 0;
		virtual void QuerySoldiersDone(const Zion::Array<SG_SOLDIER>& soldiers) = 0;
		virtual void QueryBagDone(Zion::Array<SG_EQUIPT_ITEM>& equipts, Zion::Array<SG_USABLE_ITEM>& usables, Zion::Array<SG_GEM_ITEM>& gems, Zion::Array<SG_MATERIAL_ITEM>& material) = 0;
		...
		virtual void QueryActionStatusResult(const _U32 *action_list, const Zion::Array<_U8>& statusVec, const _U32 *available_list, _U32 count) = 0;
	...
	public:
		...
		const Zion::Array<SG_GENERAL>& GetGenerals();
		const Zion::Array<SG_SOLDIER>& GetSoldiers();
		const Zion::Array<SG_EQUIPT_ITEM>& GetEquiptItem();
		const Zion::Array<SG_GEM_ITEM>& GetGemItem();
		const Zion::Array<SG_USABLE_ITEM>& GetUsableItem();
		const Zion::Array<SG_MATERIAL_ITEM>& GetMaterialItem();
		const Zion::Array<SG_INSTANCE_INFO>& GetInstanceInfo();
		const SG_SERVER_INFO& GetCurrentServerInfo();

		const int GetServerTime();
		SG_ITEM* GetItemByUUID(const A_UUID& uuid);
		void GetFinishedQuest(Zion::Array<SG_QUEST_LIVE_INFO>& quest_vec);

		void GetNewSoldierList(Zion::Array<_U32>& soldier_lists);
		bool HasNewSoldier();
		bool IsNewSoldier(_U32 soldier_id);
		void ClearNewSoldierList();

		void GetNewItemList(Zion::Array<A_UUID>& item_lists);
		bool HasNewItem();
		bool IsNewItem(const A_UUID& uuid);
		void ClearNewItemList();

		void GetNewApplyerList(Zion::Array<_U32>& applyer_list);
		bool HasNewApplyer();
		bool IsNewApplyer(_U32 applyer_id);
		void ClearNewApplyerList();

		void SyncForInit();
		void SyncSet(const Zion::Array<_U8> vecSync);
		void SyncSuccNotify();

	private:
		CSGClientCallback* m_callback;
		SG_PLAYER m_player;
		Zion::Array<SG_GENERAL> m_generals;
		Zion::Array<SG_SOLDIER> m_soldiers;
		Zion::Array<SG_EQUIPT_ITEM> m_equipts;
		Zion::Array<SG_USABLE_ITEM> m_usables;
		Zion::Array<SG_GEM_ITEM> m_gems;
		Zion::Array<SG_MATERIAL_ITEM> m_materials;
		Zion::Array<SG_QUEST_LIVE_INFO> m_quests;
		Zion::Array<SG_INSTANCE_INFO> m_instances;

		int m_nServerTimeDelta;
		_U64 m_nConnectPingTime;

		Zion::Array<SG_SERVER_INFO> m_serverList;		//目前的server
